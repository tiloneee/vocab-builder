<template>
    <div class="p-6 bg-white shadow-md rounded-lg">
      ...
      <div v-else class="space-y-4">
        ...
        >
          <div>
            <div class="font-semibold text-gray-700">{{ getLanguageName(translation.sourceLang) }}: {{ translation.originalText }}</div>
            <div class="text-gray-600">{{ getLanguageName(translation.targetLang) }}: {{ translation.translatedText }}</div>
            <div class="text-sm text-gray-400">{{ formatDate(translation.createdAt) }}</div>
          </div>
          <button
            @click="$emit('delete', translation._id)"
            class="p-2 w-10 rounded-full bg-red-100 hover:bg-red-200 text-red-600 focus:outline-none focus:ring focus:ring-red-300"
          >
